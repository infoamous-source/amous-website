      ...
      if (images.length > 1) {
        const interval = setInterval(() => {
          setCurrentIndices((prev) => ({
            ...prev,
            [affiliate.id]: ((prev[affiliate.id] || 0) + 1) % images.length,
          }));
        }, 4000);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {affiliates.map((affiliate, index) => {
            const images = affiliate.images || [];
            const currentIdx = currentIndices[affiliate.id] || 0;
            ...
            return (
              ...
              >
                ...
                <div className="flex items-start gap-4 mb-6">
                  ...
                  >
                    <ServiceIcon name={affiliate.icon || "building"} className="w-7 h-7" />
                  </div>
                  <div>
                    <h3 className="text-xl font-bold text-gray-900">{affiliate.name}</h3>
                    <p className="text-sm text-gray-500">{affiliate.name_en}</p>
                  </div>
                </div>
                  ...
                  {images.length > 0 ? (
                          ...
                          {images.map((_, idx) => (
                            <button
                              ...
                              className={`w-2 h-2 rounded-full transition-all ${
                                idx === currentIdx ? "bg-white w-6" : "bg-white/50"
                              }`}
                            />
                          ))}
                        </div>
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="text-center text-gray-400">
                        ...
                        <p className="text-xs">
                          {index === 0 ? "건축 CG 포트폴리오 이미지" : "행사 현장 이미지"}
                        </p>
                        <p className="text-xs text-gray-300 mt-1">Admin에서 등록해주세요</p>
                      </div>
                    </div>
                  )}
                </div>
